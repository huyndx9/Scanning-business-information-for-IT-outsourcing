...
여러 기업을 체크(행 앞 체크박스, 또는 현재 필터 기준 전체 선택)하면 하단 바의 **리드로
추가**로 한 번에 CRM에 보내거나(이미 리드가 있는 기업은 중복 생성 없이 "N개 추가 · M개는
이미 있음"으로 안내) **삭제**할 수 있습니다.
...
### 12b. 어디서 막히고 얼마나 걸리나 — 단계별 전환율

![단계별 전환율](docs/screenshots/20-crm-funnel.png)

상태를 바꿀 때마다 이력(`lead_status_history`)이 쌓이므로 **단계별 전환율**(신규 → 접촉 →
미팅 → 제안 → 협상 → 수주), 어느 단계에서 실패/보류로 빠졌는지, 각 단계 **평균 체류
일수**, 그리고 **세일즈 속도**(리드 생성 → 수주 평균 일수, 수주율 카드 아래 표시)를
계산합니다. 테이블의 상태 아래에는 "N일째"가 붙고 14일을 넘기면 주황색으로 바뀝니다.
리드를 직접 추가할 때 같은 회사·이메일·도메인의 리드가 이미 있으면 중복 생성 전에
확인을 요청합니다.
...
같은 데이터를 상태별 열로 봅니다. 카드(⋮⋮ 손잡이)를 다른 열로 끌면 상태가 바뀌고
즉시 저장됩니다. 카드에는 담당 영업, 다음 액션과 날짜, 금액, 점수(근거 툴팁)가 있고
열마다 `N건 · ₩X억` 합계가 붙습니다 — "미팅 단계에 얼마가 묶여 있나?"에 바로 답합니다.
빈 열은 좁아지고 **실패 · 보류**는 맨 끝에 세로로 접혀 있어 눌러서 펼치거나 바로 끌어
넣을 수 있습니다.

테이블에서는 검색·빠른 필터·상태·담당이 한 줄로 고정된 바에 있고, **기술스택 열은 기본
숨김**(회사명 툴팁으로 표시, **열** 버튼으로 선택·브라우저에 저장)이라 13인치 노트북에
맞습니다. 행에 마우스를 올리면 **📞 전화 · ✉️ 메일 초안 · 💬 카카오톡 ID 복사 · 📝 활동
기록** 버튼이 나타나고, 스캐너 배지는 해당 스캔 결과를 열며, KPI 카드를 누르면 필터가
적용됩니다. 여러 리드를 선택해 상태 일괄 변경·삭제도 가능합니다.
...
**고객** 화면(CRM 탭 → 파이프라인 | 고객, URL `/crm/customers`)은 수주 이후 단계입니다.
계약한 회사의 리드에는 파이프라인 표에서 바로 **기존 고객 · 등급** 라벨이 붙고, 리드를
열면 "기존 고객 · 등급 · 누적 금액 · 다음 만료 → 계약 보기" 띠가 보여 지금 대화 중인
상대가 기존 고객임을 바로 알 수 있습니다. CRM에서 리드를 **수주**로 바꾸면 바로 계약
...
frontend/customers.html    CRM 탭의 "고객" 화면 (계약 고객)
...
헤더 오른쪽에 탭이 있습니다. 앞의 두 탭은 *스캐너 결과*, CRM 탭은 *영업*이며 제목 아래에
**파이프라인 | 고객** 보조 탭이 있습니다.

| 탭 | URL | 내용 |
|---|---|---|
| 새 스캔 | `/` | URL 입력, 스캔(단일·일괄), 결과 확인, 데이터베이스 저장 |
| 저장된 기업 | `/saved` | 데이터베이스에 저장된 전체 기업 목록, IT 채용 추이 |
| CRM → 파이프라인 | `/crm` | 리드: 점수, 상태, 활동, 메일 초안, 가져오기/내보내기 |
| CRM → 고객 | `/crm/customers` | 계약 고객: 금액, 기간, 등급, 재계약 기회 |

"저장된 기업" 탭의 숫자는 기업 수, "CRM" 탭의 숫자는 리드 수이며 보조 탭에 리드 수와 고객
수가 함께 표시됩니다. 이전 주소 `/customers`는 `/crm/customers`로 자동 이동합니다.
...
| GET | `/api/leads/stats` | 퍼널: 단계별 도달·전환율·평균 체류일·이탈 단계·세일즈 속도 |
| GET | `/api/leads/duplicates?company_name=&email=&website=` | 회사명 / 이메일 / 도메인이 같은 기존 리드 |
